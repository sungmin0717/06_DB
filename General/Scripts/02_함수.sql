...
-- 문자열에서 5번 부터 검색 시작해서 두번쨰로 찾은 'B'의 위치 조회
SELECT 'AABAACAABBAA', 
...
FROM DUAL; --5.42 == 개월 13일

/*MONTHS_BETWEEN이 지정된 두 날짜 사이의 차를 계산할 떄 
 * 훨씬 좋다
 * -> 달 마다 길이 (28,29,30,31)가 다 다르기 때문에
 * 직접 계산하면 오차가 있을수 있는데
 * MONTH_BETWEEN 을 이용하면 이런 부분까지 모두
 * 
 * */
...
-- 자릿수 구분(,)
SELECT
	123456789, TO_CHAR(123456789, '999,999,999')
FROM DUAL;
...
SELECT TO_CHAR(CURRENT_DATE,
'YY"년" MM"월" DD"일" DAY PM HH24"시" MI"분" SS"초"')
--	'YY년 MM월 DD일 DAY PM HH시 MI분 SS초') 실패
--  날짜 형식이 부적합합니다
--> 년,월,일,시,분,초는 시간 관련 기호로 인식되지 않아서 오류 발생!!
--> [해결 방법] : "" 로 감싸서 패턴을 나타내는 기호가아닌
--  							있는 그대로 출력하는 글자임을 명시.
FROM DUAL;
...
-- 50 이상 확인
SELECT 
	TO_DATE('50-12-25', 'YY-MM-DD'), -- 2050 YY 현재세기
	TO_DATE('50-12-25', 'RR-MM-DD')  -- 1950 이전세기
FROM DUAL;
...
SELECT SUM(SALARY)
FROM EMPLOYEE -- 23행 
WHERE DEPT_CODE = 'D6'; --조건에 맞는 행만 골라서 추출
--13100000
...
-- 전체 조회 결고 행 중에서
-- PHONE 컬럼에 값이 작성된 행의 개수 카운트에서 반환.
SELECT COUNT(PHONE) --NULL 을 뺸
FROM EMPLOYEE;
