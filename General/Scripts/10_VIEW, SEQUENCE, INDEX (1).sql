...
-- VIEW 생성 권한 부여 
GRANT CREATE VIEW TO KH_KSM;

--KH 계정 접속
CREATE VIEW V_EMP
AS SELECT  * FROM EMPLOYEE;

--사번, 이름, 부서명, 직급명을쉽게 조회하기 위한 VIEW 생성.
...
-- 값이 INSERT됨을 확인!!
		...
		--> 하지만 모든 컬럼 값이 INSERRT된것이 아니라 
		-- VIEW를 생헝할 때 사용된 컬럼에만 값이 INSERT되어
		-- 반대로 사용되지 않은 컬럼에는 NULL 이 들어감
...
-- 08002: 시퀀스 SEQ_TEST_NO.CURRVAL은 이 세션에서는 정의 되어 있지 않습니다
--> 발생 원인 : CURRBAL의 정확한 의민는
...
--08004: 시퀀스 SEQ_TEST_NO.NEXTVAL 
-- exceeds MAXVALUE은 사례로 될 수 없습니다
--> MAXVALUE 150 보다 증가할수없다!
...
--SEQ_TEST_NO의 MAXVALUE 값을 200 으로 수정해보자
...
CREATE TABLE TB_IDX_TEST(
	TEST_NO NUMBER PRIMARY KEY, -- 자도으로  NIQUE INDEX 생성됨
