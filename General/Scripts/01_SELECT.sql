...
/* SYSDATE/ CURRENT_DATE, SYSTIMESTAMP / CURRENT"_TIMESTAMP*/
 ...
 * 
 * 
 m                                                     * - CURRENT_TIMESTAMP : 시스템이 나타내고 있는 현재 시간
 * 	-> ms 단위 + 지역 포함
 * 
 * */
...
--EMPLOYEE 테이블에서
--모든 사원의 사번, 이름, 입사일, 현재 까지 근무 일수 n년차조회
...
/*DISTINCT(별개의, 전혀 다른)
 * 
 * - 조회 결과 집합(RESUT SET)에
 * 	DISTINCT가 지정된 컬럼 중 중복되는 값이 존재할 경우
 * 중복을 제거하고 한번만 표시할 떄 사용하는 구문(연산자)
 * */
...
--EMPLOYEE 테이블에 존재하는 직급코드 (JOP_CODE)
...
WHERE SALARY >= 3000000
	AND SALARY < 5000000; -- 16

-- EMPLOYEE 테이블에서
-- 급여가 300만 미만 500만 이상이 사원이
-- 사번, 이름, 급여 조회
...
-- 컬럼 값이 A 이상 B 이하가 아닌 경우 TRUE(조회하겠다.)
-- == A미만 B 초과.
---------------------------------------------------
...
------------------------------------------------------
/* 컬럼명 NOT IN (값1, 값2, 값3, ...) */
-- 컬럼 값이 NOT IN () 내에 존재하지 "않으면" TRUE --> 조회 결과에 포함
--   == 연속으로 OR 연산을 작성한 것과 같은 효과
------------------------------------------------------
...
--부서코드가'D5', 'D6', 'D9'이 아닌 사원 조회 (NULL 포함.)
SELECT EMP_ID, EMP_NAME,DEPT_CODE
FROM EMPLOYEE
...
---------------------------------------------------------------------
	 ...
	 /******************LIKE(비슷한, 같은)*********************
	  * 
	  * - 비교 하려는 값이 특정한 패턴을 만족 시키면 조회하는 연산자.
	  * 
	  * [작성법]
	  * 
	  * WHERE 컬럼명 LIKE '패턴'
	  ...
	  * '_' (글자 수)
	  * 
	  * - 'A___'(_(3개)) : A로 시작하고 뒤에 3글자만 있는 문자열
	  * 					EX) ABCD :(O) , ABCDE(X)
	  * 
	  * 	'___A' : 아ㅠ에 3글자만 있고 A로 끝나는 문자열
	  * */
...
WHERE EMP_NAME LIKE '전%';
	 ...
	 -- EMPLOYEE 테이블에서 성이 '수'이름이 수로 끝나는 사원의 
	 --사번 이름 조회
 ...
 * 
 * ESACAPE OPTION : 와욀드 카드가 아닌 단순 문자열로 인식.
 * -> 지정된 특수문자 뒤 "한 글자" 에만 적용된다
 * EX) WHERE 컬럼명 LIKE '__*_'EXCAPE '*';
 * -> * 뒤에 '_' 는 일반 문자열로 인식된다.
 * 
 * */
 ...
 * 		컬럼명 | 별칭  | 컬럼순서(숫자)
 * 		[ASC / DESC]   (오름차순/내림차순)
 * 		[NULLS FIRST  / NULLS LAST]  (NULL 위치 저장)
 * 
 * *** OREDER BY절은 무조건 SELECT 마지막에 수행된다 !!! ***
 * */
...
-- WHERE 절은 SELECT 절보다 먼저 해석 되는데
-- SELECT절이 해석되어야 등록되기 떄문에
-- WHERE절에서는 별칭 사용 불가능!!!
...
ORDER BY DEPT_CODE ASC;
--> SELECT 절에 작성되지 않아도
-- 이전에 FROM절이해석되어있기 떄문에 정렬 가능!!!


/*NULLS FIRST / NULLS LAST  확인*/

/*오름 차순  ASC NULLS FIRST;*/
SELECT
	EMP_ID,
	EMP_NAME, 
	DEPT_CODE
...
-- EMPLOYEE 테이블에서 
-- 이름,부서코드,직급코드를
-- 부서코드 오름차순,직급코드 니림차순 ,이름 오름차순 정렬
