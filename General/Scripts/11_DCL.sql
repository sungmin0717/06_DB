 ...
 * 			접근(제어) 권한을 부여 (GRANT), 회수 (REBOKE)하는 언어
 * */
 ...
 * 
 * 
 * RESOURCE(자원) : 8개 객체 제어 구너한을 묶어둔 ROLE 
 * 
 * CONNECT(접속)  :  DB 접속 권한
 * 
 * */
...
/*	TEST_USER 계정 접속 */
--> SELECT 권한을 부여 받은 KH계정의 EMPLOYEE 테이브 조회.
SELECT * FROM KH_KSM.EMPLOYEE;



SELECT * FROM KH_KSM.DEPARTMENT;
--ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
		--> 조회 권한이 없어서 테이블 존재 여부 조차 모르기 떄문에 발생하는 에러
-----------------------------------------------------------
/*KH 계정 접속*/
...
DROP USER TEST_USER;
-- 01940: 현재 접속되어 있는 사용자는 삭제할 수 없습니다
