...
AS SELECT * FROM DEPARTMENT; --테이블 만들기
...
ALTER TABLE DEPT_COPY
ADD (LNAME VARCHAR2(30) DEFAULT '한국'); -- 아무것도 넣지 않았다면 한국이라고 넣을것이다.


SELECT * FROM DEPT_COPY;
--> 새로 추가된 LNBAME 컬럼에 기본값 '한국'이 자동으로 세팅됨.
...
ALTER TABLE DEPT_COPY
DROP COLUMN DEPT_ID; -- 정의가 꺠지므로 마지막 컬럼은 삭제 불가
...
--DEPT_COPY 테이블에 DEPY_ID 컬럼 갑으로 'D0'
...
--ROLLBACK을 수행해서 트랜잭션에 저장된 DML다 지우기
...
--TB2 테이블 생성 (TB1 과 관계설정(FK))
CREATE TABLE TB2(
	TB2_PK NUMBER PRIMARY KEY,
	TB2_COL NUMBER
		REFERENCES TB1(TB_1PK) 	
);
...
-- 해결 방법1) 참조하는 자식 테이블 (TB2)를 삭제하거나 FK 제약조건을 삭제

-- 해결 방법2) CASCADE CONTRAINTS 옵션 사용하기.
