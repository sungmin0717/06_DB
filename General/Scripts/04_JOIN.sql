...
ON (DEPT_CODE = DEPT_ID);
	--> DEPT_CODE와 DEPT_ID 값이 같은행을 한 줄로 합쳐라.
...
SELECT
	...
	LOCAL_NAME 지역명
FROM
 DEPARTMENT JOIN LOCATION
ON 
	LOCATION_ID = LOCAL_CODE;

--> INNER JOIN이 JOIN의 기본값 (생략 가능)
-- 오라클 방식
SELECT
	DEPT_TITLE 부서명,
	LOCAL_NAME 지역명
...
--부서명이 '해외영업2부' 인 사원의
-- 사번 EMPID 이름 EMP NAME 부서명 DEPT-TITLE 을
-- 사번 오름차순으로 조회.
...
--> JOB_CODE 컬럼명이 같아서 어떤 테이블의 컬럼이지 구분할 수 없기 떄문에.
--> 해결방법 : JOB_CODE앞에 테이블명 또는 별칭 작성
-- 오라클 -> 별칭 사용
-- 테이블 별로 별칭을 등록할 수 있음.
...
-- INNER JOIN 결과에 포함되지 못했떤
--> EMPLOYEE 테이블의 하동운, 이오라ㅣ
--> DEPRTMENT 테이블의 D3,D4,D5
-- 모두를 JOIN 결고ㅓ에 포함 시켜라.
-- == LEFT UNION (합 집합) RIGHT와 같은 결과
...
--> 직접 사용 보단 NATURAL JOIN의 실패 결과로보여지는 모습이다!
-- 라고 알아두는게 좋음
...
FROM 
	EMPLOYEE EMP LEFT JOIN EMPLOYEE MGR
ON 
...
-- EMPLOYEE , JOB, DEPARTMENT, LOATION 4개 테이블 JOIN
...
SELECT 
	...
	LOCAL_NAME,
	SALARY
FROM EMPLOYEE E, JOB J, LOCATION, DEPARTMENT
WHERE (E.JOB_CODE = J.JOB_CODE)
