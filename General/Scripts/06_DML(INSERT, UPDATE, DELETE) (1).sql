...
--SELECT 도 DML에 포함되긴 하지만 DQL이라는 것으로 분리해도 보기도한다!!
	--> DQL(DATA QUERT LANGUAGE : 데이터 질의 언어)
...
SELECT
	...
	BONUS
FROM 
	EMPLOYEE2 JOIN DEPARTMENT2 ON (DEPT_CODE = DEPT_ID)
...
-- 테이블에서 지정하는 조건의 값이 존재하면 UPDATE == KEY값 중복시 VALUE 수정
-- 조건의 값이 없으면 INSERT됨 == OBJECT 
...
WHEN MATCHED THEN --중복확인용 KEY일치 매치되면		-- 중복확인용 KEY
UPDATE SET -- 수정하겠다
...
WHEN NOT MATCHED THEN -- 중복이 되지 안으면
INSERT  -- 삽입하겠다
