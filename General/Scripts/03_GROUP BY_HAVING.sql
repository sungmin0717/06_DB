...
SELECT
	...
	SUM(SALARY)
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DEPT_CODE; -- 같은 DEPT_CODE끼리 그룹을 지음.
...
SELECT 
	DEPT_CODE,
	COUNT(*) 							-- 4. 조회 결과에 포함될 컬럼지정
FROM 
	EMPLOYEE 							-- 1 어떤 테이블에서
WHERE 
	BONUS IS NOT NULL 		-- 2. 어떤 행만 조회할지 지정
GROUP BY 
	DEPT_CODE							 -- 3. WHERE 결과에서 그룹 구분
ORDER BY
	DEPT_CODE ASC; 					-- 5. 조회결과 정렬
...
GROUP BY 
	DEPT_CODE,
	JOB_CODE -- 1) 부서별 그룹 후 2) 직급별 그룹 묶음
ORDER BY 
	DEPT_CODE ASC;
...
-- 부서별 평균이 급여 3500000원 이상인 부서를 조회하여 
--부서코드 오름차순으로 정렬
...
GROUP BY
	JOB_CODE
ORDER BY 
	JOB_CODE; -- 4 행 조회 (0명은 조회 결과에 포함 X)
-----------------------------------------------------------------
